...
bool spfa(record* edge, int flow[], vector<int> & path, int &ans, int pointer[], int n, int s,int t,map<int,int> &valid_server) {
	int  aug, k, p;
 ...
 * int &pointer[maxn]
 * */
bool findCost(record* edge, int flow[], vector<vector<int>> &paths, map<int,int> &valid_server, int pointer[], int customer_demand, int n, int s,int t,int &transfer_cost){
	//参数重置
	transfer_cost = 0;
	paths.clear();
	valid_server.clear();
...
//		cout<<"spfa:"<<count++<<endl;
		flag = spfa(edge, flow , path, transfer_cost, pointer,n,s,t,valid_server);
		if (flag==true){
