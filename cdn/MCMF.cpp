...
bool spfa(record* edge, int flow[], vector<int> & path, int &ans, int pointer[], int n, int s,int t,map<int,int> &valid_server, map<int,vector<int>> &valid_node,map<int,vector<int>> &valid_agency) {
	int  aug, k, p;
    ...
    if(valid_agency.find(agency)!=valid_agency.end()){
    	vector<int> nodes;
    	nodes.push_back(server);
    	valid_agency[agency] = nodes;
    }
    else{
    	valid_agency[agency].push_back(server);
    }
    return true;
}
 ...
 * int &pointer[maxn]
 * */
int findCost(record* edge, int flow[], vector<vector<int>> &paths, map<int,int> &valid_server, map<int,vector<int>> &valid_node, map<int,vector<int>> &valid_agency,int pointer[], int customer_demand, int n, int s,int t,int &transfer_cost){
	//参数重置
	transfer_cost = 0;
	paths.clear();
	valid_server.clear();
...
//		cout<<"spfa:"<<count++<<endl;
		flag = spfa(edge, flow , path, transfer_cost, pointer,n,s,t,valid_server,valid_node,valid_agency);
		if (flag==true){
